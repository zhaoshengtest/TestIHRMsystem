...
class TestEmp(unittest.TestCase):
    ...
    def test02_add_emp(self):
        # 实现添加员工接口
        response = self.emp_api.add_emp("特朗普xx2x","14623456789",app.HEADERS)
        # 打印添加的结果
    ...
    def test04_modify_emp(self):
        # 实现修改员工接口
        # 发送修改员工的接口请求
        response = self.emp_api.modify_emp(app.EMP_ID,"印度阿3",app.HEADERS)
        # 打印修改员工的结果
        ...
        conn.close()
        # 断言数据库查询的结果
        self.assertEqual("印度阿3", result[0])
        # 断言
        assert_common_utils(self, response, 200, True, 10000, "操作成功")
